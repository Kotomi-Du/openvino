...
\
        ...
        auto transpose_before_reshape_const_m = wrap_type<ov::op::v0::Constant>();\
        auto transpose_before_reshape_m = wrap_type<ov::op::v1::Transpose>({mul_m, transpose_before_reshape_const_m});\
        auto reshape_after_transpose_const_m = wrap_type<ov::op::v0::Constant>();\
        auto reshape_after_transpose_m = wrap_type<ov::op::v1::Reshape>(\
            {transpose_before_reshape_m, reshape_after_transpose_const_m}, reshape_squeeze);\
\
        auto compressed_weights_input_m =\
            std::make_shared<ov::pass::pattern::op::Or>(\
                ov::OutputVector{reshape_m, convert_reshape_m, transpose_m, reshape_after_transpose_m, mul_m, mul2_m});
