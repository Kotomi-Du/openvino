...
TransposeSDPAMatcher::TransposeSDPAMatcher() {
    ...
    ov::matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](Matcher& m) {
        ...
        auto select_kv_input = [&](const std::shared_ptr<ov::Node>& float_pattern,
        ...
        auto input_k = select_kv_input(input_k_m, input_k_output_idx, 1);
        auto input_v = select_kv_input(input_v_m, input_v_output_idx, 2);
        ...
        auto sdpa_new = std::make_shared<op::SDPA>(inputs,
                                                   sdpa->get_causal(),
                                                   order_q,
                                                   order_k,
                                                   order_v,
