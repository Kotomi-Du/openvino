...
ConvertFullyConnectedToFullyConnectedCompressed::ConvertFullyConnectedToFullyConnectedCompressed() {
    ...
    ov::matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](ov::pass::pattern::Matcher& m) {
        ...
        const bool has_transpose_before_reshape = pattern_map.count(reshape_after_transpose_m) != 0u;
        const bool has_transpose = pattern_map.count(transpose_m) != 0u || has_transpose_before_reshape;
        ...
        auto reshape_decompression_input = [&](std::shared_ptr<ov::Node> node) -> std::shared_ptr<ov::Node> {
        ...
        auto convert_to_u8 = [&](std::shared_ptr<ov::Node> node) {
        ...
        if (with_zero_point) {
            ...
            optional_zero_point = convert_to_u8(zero_point);
        }
        ...
        if (has_transpose) {
            const auto& transpose = pattern_map
                                        .at(has_transpose_before_reshape ? transpose_before_reshape_m : transpose_m)
                                        .get_node_shared_ptr();
            std::shared_ptr<ov::Node> transpose_const =
                pattern_map
                    .at(has_transpose_before_reshape ? transpose_before_reshape_const_m : transpose_const_m)
                    .get_node_shared_ptr();
        ...
        if (has_transpose_before_reshape) {
            const auto& reshape = pattern_map.at(reshape_after_transpose_m).get_node_shared_ptr();
            const auto& reshape_const_node = pattern_map.at(reshape_after_transpose_const_m);
            fc_input_b = reshape->clone_with_new_inputs({fc_input_b->output(0), reshape_const_node});
            result_nodes.push_back(fc_input_b);

            auto reshape_decompression_parameter = [&](std::shared_ptr<ov::Node> node) {
                const auto& shape = node->get_output_shape(0);
            ...
            fc_input_scale = reshape_decompression_parameter(fc_input_scale);
            if (with_zero_point) {
                fc_input_zp = reshape_decompression_parameter(fc_input_zp);
            }
        }
